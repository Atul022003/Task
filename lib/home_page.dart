...
import 'package:task/provider/auth_provider.dart';
import 'package:task/provider/product_provider.dart';
import 'package:task/product_detail_page.dart'; // Import ProductDetailPage
...
class HomePageState extends State<HomePage> {
      ...
      body: Column(
        children: [
          ...
          (provider.allProducts.isNotEmpty)
              ...
              shrinkWrap: true,
              children:
              provider.allProducts.map((product) => ProductWidget(product, provider)).toList())
              : Column(
            children: [
              Icon(Icons.list),
              Text("No Products Found")
  ...
  Widget ProductWidget(Product product, ProductProvider provider) {
    return GestureDetector(
      onTap: () {
        // Navigate to ProductDetailPage when product is tapped
        Navigator.push(
          context,
          MaterialPageRoute(
            ...
            child: Column(
              children: [
                ...
                (product.imagePath.isNotEmpty)
                    ? Image.file(File(product.imagePath), height: 100)
                    : SizedBox(),
                SizedBox(height: 10),
                Text(
                  product.name,
